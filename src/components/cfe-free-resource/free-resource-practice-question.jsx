...
const FreeResourcePracticeQuestion = () => {
    ...
    switch (moduleCode) {
      case "CFE-1":
        return {
          ...
          questions: questionsData.filter(q => q.questions_module === "CFE-1")
        };
      case "CFE-2":
        return {
          id: 2,
          title: "MODULE-II: LAW",
          questions: questionsData.filter(q => q.questions_module === "CFE-2")
        };
      case "CFE-3":
        return {
          id: 3,
          title: "MODULE-III: INVESTIGATION",
          questions: questionsData.filter(q => q.questions_module === "CFE-3")
        };
      case "CFE-4":
        return {
          id: 4,
          title: "MODULE-IV: FRAUD PREVENTION & DETERRENCE",
          questions: questionsData.filter(q => q.questions_module === "CFE-4")
        };
      default:
        return {
          id: 1,
  ...
  const getMoreQuestionsLink = (moduleId) => {
    const moduleCode = moduleId === 1 ? "CFE-1" : 
                     moduleId === 2 ? "CFE-2" :
                     moduleId === 3 ? "CFE-3" : "CFE-4";
                  ...
                  {showAnswerFor === currentQuestion.id && (
                    <div className="bg-[#0F3652]/5 p-4 rounded-lg">
                      ...
                      <p className="text-[#0F3652] text-xs leading-relaxed">
                        <span className="font-semibold">{currentQuestion.optionCorrect}</span>
                        <br />
